...
function StatBlock({ icon, label, value, color }) {
  return (
    <div style={{
      flex: 1, padding: 14, background: '#fff', border: '1px solid #e2e8f0',
...
export default function CourseAnalyticsTab({ user, courseId }) {
  ...
  // Engagement (last 14 days, fill missing days)
  ...
  if (loading) {
    return (
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(360px, 1fr))', gap: 16 }}>
        {[1, 2, 3, 4].map(i => (
          <div key={i} style={{ height: 320, background: '#f1f5f9', borderRadius: 12 }} />
        ))}
      </div>
    )
  }
  ...
  if (!data || data.completion?.totalEnrolled === 0) {
    return (
      <div style={{
        ...
        <BarChart3 size={48} color="#cbd5e1" style={{ margin: '0 auto 12px' }} />
        <h3 style={{ margin: '0 0 6px', fontSize: 16, fontWeight: 600, color: '#475569' }}>
          No analytics yet
        </h3>
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip
                  ...
                  contentStyle={{ fontSize: 12, borderRadius: 8 }}
                />
                <Bar dataKey="completionRate" fill="#8b5cf6" radius={[0, 6, 6, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
